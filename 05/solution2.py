    ...
    for line in lines:
        ...
        if(y1 == y2 and x1 < x2):
            ...
            for y in range(y2,y1):
                visitedpoints[x1][y] += 1
        #diagonal case where both x1 and y1 are increasing
        elif(x1 < x2 and y1 < y2):
            while(x1 <= x2 and y1 <= y2 ):
                visitedpoints[x1][y1] += 1
                x1 += 1
                y1 += 1
        #diagonal case where both x1 and y1 are decreasing
        elif(x1 > x2 and y1 > y2):
            while(x1 >= x2 and y1 >= y2):
                visitedpoints[x1][y1] += 1
                x1 -= 1
                y1 -= 1
        #diagonal case where x is increasing and y is decreasing
        elif(x1 < x2 and y1 > y2):
            while(x1 <= x2 and y1 >= y2):
                visitedpoints[x1][y1] += 1
                x1 += 1
                y1 -= 1
        #diagonal case where x is decreasing and y is increasing
        elif(x1 > x2 and y1 < y2):
            while(x1 >= x2 and y1 <= y2):
                visitedpoints[x1][y1] += 1
                x1 -= 1
